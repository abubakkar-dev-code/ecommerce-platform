...
const Cart = () => {
  ...
  useEffect(() => {
    const getCart = async () => {
      try {
        ...
        setTotalAmount(cartItems.data.totalAmount || 0);
      } catch (err: any) {
  ...
  const handleRemove = async (varientId: string) => {
    try {
      ...
      setTotalAmount(updatedCart.data.totalAmount || 0);
    } catch (err: any) {
            ...
            {items.length === 0 ? (
              ...
              <div className="space-y-4">
                <button
                  ...
                  className="mb-4 rounded-md border border-error px-4 py-2 text-sm font-medium text-error hover:bg-error hover:text-white disabled:opacity-50"
                >
                  {loading ? "Updating..." : "Clear Cart"}
                </button>
                {items.map((item) => (
                  <div
                  ...
                  >
                    <div className="flex gap-6">
                      {/* Product Image */}
                      <img
                        src={item.product.images?.[0]?.image || "https://placehold.co/150x150?text=" + encodeURIComponent(item.product.name)}
                    ...
                    <button
                      ...
                      className="mt-4 rounded-md border border-error px-4 py-2 text-sm font-medium text-error hover:bg-error hover:text-white disabled:opacity-50"
                    >
                      Remove
                    </button>
                  </div>
